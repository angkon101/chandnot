...
export default function Toolbar({ editor, onImageUpload }: ToolbarProps) {
  ...
  const btn = (active: boolean, title: string, onClick: () => void, children: React.ReactNode) => (
    <button
      ...
      className={`px-2 py-1 text-xs font-mono transition-all shrink-0 rounded ${
        active
          ? 'bg-cyan-400/15 text-cyber-cyan shadow-none'
  ...
  const divider = <div className="w-px h-4 bg-white/5 mx-1 shrink-0" />

  return (
    <div className="overflow-x-auto">
      <div className="flex items-center gap-0 px-3 sm:px-4 py-1.5 border-b border-white/5 bg-black/10 min-w-max">
        ...
        {divider}
        <div className="flex items-center gap-0 px-1 bg-white/[0.02] rounded">
        ...
        <button type="button" title="Image" onClick={onImageUpload} className="px-2 py-1 text-xs text-white/30 hover:text-white/60 hover:bg-white/[0.03] transition-all font-mono shrink-0 rounded">
          Img
        </button>
        {divider}
        <button type="button" title="Undo" onClick={() => editor.chain().focus().undo().run()} disabled={!editor.can().undo()} className="px-2 py-1 text-xs text-white/20 hover:text-white/50 hover:bg-white/[0.03] disabled:opacity-15 transition-all font-mono shrink-0 rounded">Undo</button>
        <button type="button" title="Redo" onClick={() => editor.chain().focus().redo().run()} disabled={!editor.can().redo()} className="px-2 py-1 text-xs text-white/20 hover:text-white/50 hover:bg-white/[0.03] disabled:opacity-15 transition-all font-mono shrink-0 rounded">Redo</button>
      </div>
    </div>
  )
}
